...
Phases 0–10 are complete and have been verified end-to-end (real crawl against a live
site, real MongoDB persistence, real browser-served frontend — not mocked).

- [x] Phase 0 — Architecture & Planning
- [x] Phase 1 — Backend Foundation (Express, config, logging, error handling, health endpoint)
- [x] Phase 2 — Database Models & Repository Layer (Audit model, AuditRepository)
- [x] Phase 3 — Crawler Engine (fetcher, URL normalizer, navigation detector, domain filter, SSRF guard)
- [x] Phase 4 — SEO Analysis Engine (all 7 rule modules, centralized issue registry)
- [x] Phase 5 — Audit Service & APIs (POST /audit, GET /audit/:id, fire-and-forget execution)
- [x] Phase 6 — Backend Tests & Hardening (69 Vitest tests across crawler/analyzer/API)
- [x] Phase 7 — React Frontend Foundation (api client, types, hooks, base components)
- [x] Phase 8 — Audit Start Flow (StartAuditForm, client-side validation)
- [x] Phase 9 — Audit Overview (metric cards, zero-issues success banner)
- [x] Phase 10 — Page-Level Breakdown (expandable table, severity indicators)
- [x] Phase 11 — Loading/Error/Empty States (dedicated review pass; see below)
- [x] Phase 12 — Frontend Polish & Accessibility (dedicated review pass; see below)
- [ ] Phase 13 — End-to-End Validation (automated; manual smoke-testing done, see below)
- [ ] Phase 14 — README + Final Documentation

### Phase 11/12 review findings and fixes

Dedicated pass over every loading/error/empty state and an accessibility sweep of
the components built in Phases 8–10. Real issues found and fixed:

- **Issue badges showed raw codes, not human-readable labels.** Spec section 4
  requires "a human-readable label" for the frontend to display. `IssueBadge` was
  rendering `TITLE_MISSING` verbatim instead of the `message` field from
  `ISSUE_DEFINITIONS`. Fixed; the raw code is now only in a `title` tooltip.
- **Severity/status colors failed WCAG AA contrast.** `amber-500`, `green-500`, and
  `blue-500` as white-text badge/text-color fills came out between 2.2:1–3.7:1
  against white — well under the 4.5:1 (or 3:1 for large text) AA minimum. Moved
  all four status colors and both severity colors to their `-700` shades
  (verified 4.8:1–7.6:1) in `utils/formatters.ts`.
- **Duplicate `<h1>` on the start screen.** The persistent app header and
  `StartAuditForm` both rendered an `<h1>`, breaking heading hierarchy for screen
  reader users. Demoted the form's heading to `<h2>`.
- **Expandable table rows used `role="button"` on a `<tr>`.** Nesting interactive
  role semantics inside native table markup is fragile across screen readers.
  Replaced with a real `<button>` in the trailing cell (`aria-expanded`,
  descriptive `aria-label`); the row's `onClick` remains for mouse convenience.
- **Validation errors weren't announced to screen readers.** `ErrorAlert` had no
  `role="alert"`, and the URL input had no `aria-describedby` linking it to the
  error text. Both added.
- **Loading spinner ignored `prefers-reduced-motion`** and had no `role="status"`
  for screen readers to announce the loading message. Fixed both; also removed
  two unused `@keyframes` (`fadeIn`, `slideIn`) that were dead CSS.
- **Browser back/forward button did nothing.** `App.tsx` synced the audit ID *to*
  the URL on state changes but never read it back on `popstate`, so using the
  back button after starting an audit left the results screen showing with a
  stale URL. Added a `popstate` listener.

States confirmed present and correct against spec section 11: invalid URL
(client-side, before any request), API/network failure on start, in-progress
message ("Audit in progress… Crawling website and analyzing pages."), failed
audit with the persisted error message, zero-issues success banner, and a
graceful message when a completed audit somehow has no pages.

### Manual verification performed

- `POST /audit` against `https://example.com` → real crawl executed, correctly detected
  no `<nav>` element, analyzed the homepage, found 3 real issues
  (`TITLE_TOO_SHORT`, `META_DESCRIPTION_MISSING`, `CANONICAL_MISSING`)
- `GET /audit/:id` while RUNNING and after COMPLETED, response shape matches the
  documented contract exactly (`audit_id`, not Mongo's raw `_id`)
- Backend: `npm run typecheck` clean, `npm test` → 69/69 passing, `npm run build` clean
- Frontend: `npm run typecheck` clean, `npm run build` clean, dev server serves all
  new screens without compile errors
- **Not verified**: actual browser rendering/interaction of the React UI (form
  submission, polling transitions, expand/collapse rows). No browser automation
  tooling was available in the dev environment used to build this — this needs a
  manual pass in an actual browser before sign-off.

### Deviations from the original plan (discovered during implementation)

1. **Task runner: `tsx` instead of `ts-node/esm`.** `ts-node`'s legacy
   `--experimental-loader` hook runs off-thread on Node 20+, and errors thrown
   inside it get corrupted in transit back to the main thread (surfacing as a
   useless `[Object: null prototype]` with no message). Switched `dev`/build
   tooling to `tsx`, which doesn't have this problem and is the current standard.
2. **Env loading: Node's built-in `--env-file` instead of `dotenv`.** Node 20.6+
   supports `--env-file=.env` natively, avoiding an extra dependency.
3. **`MONGODB_URI` validation is a regex, not `z.string().url()`.** Atlas's
   standard (non-SRV) connection string format is a comma-separated multi-host
   authority (`mongodb://host1:port,host2:port,host3:port/db?...`), which the
   WHATWG URL parser — and therefore Zod's `.url()` — rejects as invalid. Needed
   the non-SRV format in the first place because this dev environment's Node
   process cannot perform DNS `SRV` lookups (`querySrv ECONNREFUSED`) even though
   the OS resolver can; this is a known corporate-network/VPN/EDR interaction and
   is documented here in case it recurs in another environment.
4. **`GET /audit/:id` response DTO.** The Mongoose document's `_id` field doesn't
   match the documented `audit_id` field. Added `toAuditResponse()` in
   `src/types/dto.ts` as the single place both endpoints map the internal `Audit`
   shape to the public API shape, rather than leaking the Mongoose field name.
5. **`RUNNING` status race in `POST /audit`.** The status transition to `RUNNING`
   is now `await`ed before the HTTP response is built (a fast DB write), so the
   returned `status` always matches what's persisted. Only the crawl/analyze
   pipeline itself is fire-and-forget, not the initial status transition.

---

## Next Steps

Remaining before Definition of Done (spec section 32):

- Phase 13 — a real browser smoke test of the full flow (see "Not verified" above),
  plus deliberate edge-case testing (site with no nav, site that 404s, oversized page)
- Phase 14 — README.md with setup instructions, architecture diagram, API docs
